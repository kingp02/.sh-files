...
choose_image() {
  ...
  if [ -f /mnt/stateful_partition/unencrypted/first_time ];then
    ...
    if [ "$first_time" = "1" ];then
      ...
      prompt "Use image $saved? 'y' (yes) or 'n' (no): "
      read multi
      if [ "$multi" = "y" ]; then
        num="$saved"
      else
        prompt "Select a recovery image to download. This will be saved for next time: "
	      read num
    ...
    if [ -n "$line" ]; then
      ...
      case $key in
        ...
        url)
          ...
          if [ "$val" = "$up_url" ]; then
	          prompt "No update available. Would you like to change your image number? 'y' (yes), 'n' (no) or 'c' (update anyway)? : "
	          read change </dev/tty
		        if [ "$change" = "n" ]; then
	    	      exit
	          elif [ "$change" = "c" ]; then
